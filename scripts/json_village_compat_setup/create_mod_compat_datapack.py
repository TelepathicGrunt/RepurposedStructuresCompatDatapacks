...
while restart:
    ...
    folderName = f"{datapack_location}\\Compat_{mod_name.replace(' ', '_')}"
    data_location = f"{folderName}\\src\\data"

    #-------------------------------------------------------------------------------------------
    
    # Setup project
    root_src_dir = os.path.join('template', 'Compat_Blank')
    for src_dir, dirs, files in os.walk(root_src_dir):
        dst_dir = src_dir.replace(root_src_dir, folderName, 1)
    ...
    # Gradle properties
    with open(os.path.join(folderName, 'gradle.properties'), 'r' ) as input:
        content = input.read()

        content_new = re.sub('$1', rf'repurposed_structures_{mod_namespace}', content, flags = re.M)
        content_new = re.sub('$2', rf'repurposed_structures_{mod_name}', content, flags = re.M)
        content_new = re.sub('$3', rf'{mod_namespace}', content, flags = re.M)
            
        with open(os.path.join(folderName, 'gradle.properties'), 'w' ) as output:
